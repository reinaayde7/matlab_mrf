...
raw = transformKspaceToImage(raw,3); % fftshift(ifft(ifftshift(raw ,3),[],3),3)*sqrt(size(raw,3)); JESUS
...
raw = flip(raw, 2); % This is to match the B0 and B1 ordering

if 1  
    fprintf('Coil Compression\n')
    raw = pcaCoilCompress(raw,0.95);
end
...
if CalculateDict,
    ...
    if ~SliceProfileCorrection
        ...
        [dict,r] = Calculate_MRF_FISP_DictwithDelays(RawDataFileName,rawinfo,t1series,t2series,0,...%[0.6:0.1:1.4] for B1
            'FISP_TE.txt','FISP_TR.txt','FISP_FA.txt','FISP_PH.txt',1,delay,4);
        ...
        NumOfFrames = 3000;
        [dict,r] = calcFISP_dict_highres(pulseduration,BandWidthTimeProduct,sliceThickness,baseTR,theta,tr,NumOfFrames,3000);
...
tt_1 = zeros(size(raw,3),6); %each of these variables is to save a through-time measurement in certain pixel JESUS
tt_2 = zeros(size(raw,3),6);
tt_3 = zeros(size(raw,3),6);
tt_4 = zeros(size(raw,3),6);
...
for nn = 1:size(raw,3),
    ...
    if lowrank_SVD
        raw = compress_kspace_svd(permute(squeeze(raw),[1,3,2]),Vc,projID,(size(kx,2)));
    ...
    if OffResonanceDeblurr
        ...
        [demod_data, deltaw] = B0_CPR_deblurr_1(raw, LPF(:,:,b0index), L);
        ...
        for iproj= 1:NumFrames
            if lowrank_SVD
                ...
                for inc=1:NumCoils
                    ...
                    k_temp = squeeze(raw(:,:,inc,iproj));  % fullly sampled data
                    image_temp = G'*(dcf(:).*k_temp(:));
                ...
                for inc=1:NumCoils
                    ...
                    k_temp = squeeze(raw(:,inc,:,iproj));  % single shot
                    image_temp = G'*(dcf(:).*k_temp(:));
                    image_uncombined(:,:,inc,iproj) = embed(image_temp,mask);
                    index_progress = (iproj-1)*NumCoils+inc;
                    %waitbar(index_progress/(NumFrames*NumCoils), f,...
