...
log.FA = importdata(fatxtfile);
log.tr0 = log.rawinfo.TR(1)/1000/1000;
log.TR = zeros([log.rawinfo.Nex, 1]) + log.tr0;
log.TE = log.rawinfo.TE * 1e-6;
fov = log.rawinfo.fieldofview;
PH = zeros(size(log.FA));
...
%brain 1 x1 x3
dictionary_name = ['dict_FISP_Chris_NIST_TE' log.rawinfo.TE '_TR' log.rawinfo.TR(1) '_Nex' log.rawinfo.Nex]
if CalculateDict
    ...
    log.t1series = [10:10:1000 1020:20:2000 2050:50:3000];
    log.t2series = [2:2:100 105:5:300 310:10:500 520:20:800 850:50:1500 1600:100:2000];
    if isTRUEFISP
    ...
    if isTRUEFISP
        tic
        [dict,r] = Calculate_MRF_TRUEFISP_DictwithDelays( DataFileName,log.rawinfo,log.t1series,log.t2series,log.offres,...%[0.6:0.1:1.4] for B1
