...
%filename = 'data/meas_MID03552_FID105839_gre_field_mapping';
...
%disp(size(data1))
data1 = data1( :, :, :, 4); % TE 1 data
data2 = data2( :, :, :, 4); % TE 2 data
...
image_combined1 = image_combined1(size(image_combined1,1)/4:3*size(image_combined1,1)/4,:); % center image and
image_combined2 = image_combined2(size(image_combined2,1)/4:3*size(image_combined2,1)/4,:); % remove oversampling
...
OffRes = Delta_Phi/Delta_T;
%{
%% Load Dicom to compare
[info] = dicominfo('PreScan.MR._.2.1.2022.06.08.12.36.46.738.69951818.dcm');
[B0map] = dicomread('PreScan.MR._.2.1.2022.06.08.12.36.46.738.69951818.dcm');
...
%  to create the mask
%load("Blurred_Image.mat","image_combined")

%C = imquantize(abs(mean(image_combined(:,:,1:15),3)),55); %mask from MRF through-time average THIS FILE IS IN THE 2D_MRF_ReconstructionMain folder
A = OffRes;
...
[U,V] = meshgrid(linspace(1,size(A,2),400), linspace(1,size(A,1),400));
%[UU,VV] = meshgrid(linspace(1,size(I,2),400), linspace(1,size(I,1),400));
%subplot(122), imagesc(F(U,V));

InterpolatedField = F(U,V);
%InterpolatedDicom = FF(UU,VV);
InterpolatedField(InterpolatedField>220)=220;
InterpolatedField(InterpolatedField<-220)=-220;
%{
...
%{
subplot(1,2,2)
imagesc(InterpolatedDicom);
colormap('gray');
title('InterpolatedDicom')
colorbar
...
save('off_res.mat','InterpolatedField')
