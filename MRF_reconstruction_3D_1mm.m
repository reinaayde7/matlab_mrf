% Sample script for demonstrating the MRF reconstruction to estimate T1 and
% T2 values.
%
...
B1correction = 0;
SliceProfileCorrection = 0; %Rudy: sim and evaluation with =1 TODO
linearID = 0;
PartialFourier = 0;
%Nex = 3000;
BinaryGT = 0;
doSave = 0;
doRRdebug = 0;
% TotalSlices = 26; %Slices without partial fourier
...
if CalculateDict
    ...
    log.t2series = [2:2:100 105:5:300 310:10:500 520:20:800];% 850:50:1200];% 310:10:500 520:20:800 850:50:1500 1600:100:2000];

    log.trtxtfile = [folderTXT '\FISP_TR.txt'];
    log.tetxtfile = [folderTXT '\FISP_TE.txt'];
    log.fatxtfile = [folderTXT '\FISP_FA.txt'];
    log.phtxtfile = [folderTXT '\FISP_PH.txt'];


    if ~SliceProfileCorrection
        if isTRUEFISP
            [dict,r] = Calculate_MRF_TRUEFISP_DictwithDelays(RawDataFileName,rawinfo,log.t1series,log.t2series,0,...%[0.6:0.1:1.4] for B1
            log.tetxtfile,log.trtxtfile,log.fatxtfile,log.phtxtfile,1,log.delay,0);
            
            save('Dict_TRUEFISP_noSPcorr.mat','dict','r','log','-v7.3');
        else
            [dict,r] = Calculate_MRF_FISP_DictwithDelays(RawDataFileName,rawinfo,log.t1series,log.t2series,0,...%[0.6:0.1:1.4] for B1
            log.tetxtfile,log.trtxtfile,log.fatxtfile,log.phtxtfile,1,log.delay,2);
            save('Dict_FISP_noSPcorr.mat','dict','r','log','-v7.3');
        end
    else
        %%% --------------------------------------------------------------------------------------
    ...
    if ~SliceProfileCorrection
        if B1correction
            ...
            if ~isTRUEFISP
                load('E:\POSTDOC_UoM\08_Project_MRF\3D_MRF_FISP_Prostate-main_d240531\Dict_FISP_noSPcorr.mat','dict','r');
            else
                load('E:\POSTDOC_UoM\08_Project_MRF\3D_MRF_FISP_Prostate-main_d240531\Dict_TRUEFISP_noSPcorr_v6_b0.mat','dict','r');
            end
        end
    else
        BandWidthTimeProduct = 8;
...
%% Prepare NUFFT
if doRRdebug
    addpath('E:\POSTDOC_UoM\02_Project_PE_lung_MRI_055T\Jesse-RealTimeCine_DeepImagePrior-main-d2310\Reconstruction_Code\');
    % --------------------------------------------------------------------
    % RUDY
    % --------------------------------------------------------------------
    [~,uplimit] = max(kxall(:,1).^2+kyall(:,1).^2) % farthest excursion in k-space
    kx = kxall(1:uplimit,:);
    ky = kyall(1:uplimit,:);
    [nr,np] = size(kx);   % nr = # of readout points, np = # projections in fully sampled data
    
    %{
    During the data acquisition, the ADC is turned on slightly before the read
    gradient starts. Here, we remove the first sampled points, where were
    acquired before the readout gradient started.
    %}
    adcpad = 20; % number of points at beginning of spiral to throw away (ADC on, but read gradient off)
    % DATA = DATA(adcpad+1:adcpad+uplimit,:,:,:);
    
    % Calculate the spiral density compensation function (DCF)
    kmax = max(sqrt(kx(:).^2+ky(:).^2)); % maximum extent of k-space
    resol = 10 / (2*kmax)                % in-plane resolution (mm)
    fov = resol * N(1);                     % field of view (mm)
    % clearvars kxall kyall
    kxall = kx./kmax*N(1)/2;                % scale trajectory within range [-N/2, +N/2]
    kyall = ky./kmax*N(2)/2;
    kspace = [kxall(:) kyall(:)]/fov;
    readoutOS = 2;                       % 2-fold readout oversampling (this is a property of the sequence! Do not change it!!)
    nufftmask = true(N(1),N(2));
    Ni=size(nufftmask);
    nufft_args = {Ni, [6 6], 2*Ni, Ni/2, 'table', 2^12, 'minmax:kb'};
    G = Gmri(kspace, nufftmask, 'fov', fov, 'basis', {'dirac'}, 'nufft', nufft_args);
    densityComp = abs(ir_mri_density_comp(kspace, 'pipe','fix_edge',2,'G',G.arg.Gnufft));
    densityComp = reshape(densityComp,size(kxall));
    %Rudy NB: monitor for spikes in DCF -> spikes at the end of DCF do not matter though!
    clear nufftmask Ni nufft_args G
    
    figure; set(gcf,'position',[66         378        1025         500]);
    subplot(121);
    plot(kxall(:,1),kyall(:,1),'.','MarkerSize',10,'Color','b');
    axis square; title('Spiral (projection #1)','fontweight','bold','fontsize',15);
    xlabel('k_x','fontsize',13); ylabel('k_y','fontsize',13);
    subplot(122);
    plot(densityComp(:,1),'linewidth',3,'Color','b');
    axis square; title('Density Compensation Function (DCF)','fontweight','bold','fontsize',15);
    xlabel('Readout','fontsize',13);
    ylabel('DCF','fontsize',13);
    drawnow
    ...
    for z = 1:nz % loop over all partitions
    ...
    error('STOP HERE - it is just for debug! (240627)')
end
%%
% Calculate nufft
if lowrank_SVD,
...
for nn = 1:10 %size(raw,3),
    ...
    if OffResonanceDeblurr
        ...
        [demod_data, deltaw] = B0_CPR_deblurr_1(raw_backup, LPF(:,:,nn), L);
...
climst1 = [0, 3000];
climst2 = [0, 1200]; %uplimit:1200 for NIST T2 at 0.55T 
sliceSTART = 1;
sliceEND = 26;
