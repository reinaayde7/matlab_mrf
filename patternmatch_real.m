...
function [t1big,t2big,adcbig,m0big] = patternmatch(imadapt,roimask,r,d,dict,nblocks, dictnorm)
...
%     prevlen = numel(msg);
    ...
    xxR = zeros(size(xx,1),size(xx,2)*2);
    xxR(:,1:size(xx,2)) = imag(xx); 
    xxR(:,size(xx,2)+1:end) = real(xx);
    dictR = zeros(size(dict,1)*2,size(dict,2));
    dictR(1:size(xx,2),:) = real(dict); 
    dictR(size(xx,2)+1:end,:) = imag(dict);
    
    innerProductR = xxR*dictR./normAll; 
    % innerProduct = conj(xx)*dict(trange,:)./normAll; %clear normAll
     
    
    % [value indexm] = max(abs(innerProduct),[],2); clear innerProduct
    % [valueR indexmR] = max(innerProductR,[],2);
    [value indexm] = max(abs(innerProductR),[],2);
